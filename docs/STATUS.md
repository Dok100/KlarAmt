...
- **Steuernummer/Aktenzeichen in den Brief (clientseitig)** ✓ (2026-06-13) — bei Steuerbescheiden blieb das Referenz-Feld leer: `stripPII` ersetzt die Steuernummer durch `[STEUERNUMMER]`, bevor Haiku den Text sieht. Neue `extrahiereGeschaeftszeichen()` in `lib/pii.ts` liest Steuernummer/Aktenzeichen aus dem **Rohtext vor** stripPII; die Analyse-Route trägt den Wert nach dem Parse in `absender.aktenzeichen` nach (nur falls leer, Text-Modus — Vision liest es direkt aus dem Bild). Datenschutz-Entscheidung des Nutzers: der Wert geht **nicht** an die KI. Der Brief enthält nur den Platzhalter `[AKTENZEICHEN]`, den `fuelleBrief` clientseitig durch `— Steuernummer/Aktenzeichen <Wert>` ersetzt (Label je Dokumenttyp); der Antwort-Call sendet das Geschäftszeichen nicht mehr (schützt nebenbei das bisher übertragene Bußgeld-Aktenzeichen). Feld-Label dynamisch („Steuernummer" bei Steuerbescheid). Beide Testdateien verifiziert (`143/250/60815`, `32.4-OWi-2026/55418`).
- **Manuelles Zustelldatum → Live-Countdown** ✓ (2026-06-13) — wenn eine zustellungsgebundene Frist ohne erkennbares Datum vorliegt (`FristHinweis`-Pfad), bietet die Box jetzt ein leeres `<input type="date">` („Zustelldatum laut Briefumschlag oder Eingangsstempel"). Bei Eingabe rechnet `berechneFristende(datum, tage, 0)` clientseitig das Fristende und zeigt einen Countdown — **ohne** Bekanntgabefiktion, da das eingegebene Datum bereits das echte Zugangsdatum ist (die Fiktion ersetzt nur ein unbekanntes). Kein API-Call, das Datum verlässt das Gerät nie. Bewusst eng: nur bei `frist_tage` gesetzt + kein `bescheid_datum`, nicht bei `zahlung`-Fristen. Fristende geht NICHT in den Brief (nur UI-Orientierung, keine Rechtsberatung). Einzige Code-Datei: `ergebnis/page.tsx` (`FristHinweis` umgebaut). `berechneFristende` war bereits exportiert/rein.
- **Behördenadresse in den Brief übernehmen** ✓ (2026-06-13) — die Empfänger-Adressfelder (Straße, PLZ/Ort) im Einspruchsformular blieben leer, obwohl die Adresse im Bescheid steht. Ursache: `absender` im Analyse-Schema hatte nur `behoerde`/`abteilung`/`aktenzeichen`, kein Adressfeld — Haiku las die Adresse nur in den Fließtext. Schema + Prompt-Template um `strasse` und `plz_ort` erweitert, im Ergebnisformular als Empfänger-Default gesetzt. Die Antwort-Route übernahm `strasse`/`plzOrt` bereits in den Brief; es fehlte nur die Extraktion. **Auf Haiku zu verifizieren.**
